...
全部是通用引擎,行业个性只存在于 `config/need_<id>.yaml` 画像里。首个实例是**国内行业安全事件库**
(每日采集国内网络/信息安全事件,形成可用于商业决策的结构化事件库);第二个实例**政策监管动态库**
(`config/need_policy_watch.yaml`,文档型)用来证明"零改引擎、只换画像"。

平台设计与验证见 `design/platform/`:
