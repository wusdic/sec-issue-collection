...
EDITABLE = [
    ...
    ("auto_column_max", "根域源自动发现栏目数", "采集", "int", False,
     "根域站点按画像的栏目提示词自动识别相关栏目分别采集,每站最多这么多个栏目。默认 8"),
    ...
    ("column_relevance_min", "栏目验证:内容相关度下限", "采集", "float", False,
     "候选栏目里标题命中本需求相关词(画像栏目提示词+关键词矩阵)的文章占比≥此值才算『相关栏目』(0-1)。"
    ...
    ("auto_to_site_search", "抓不到的页面源自动转站内检索", "采集", "bool", False,
     "页面源被自动停用时,顺手建一个『站内检索』兄弟源(借搜索引擎按 site:域名 抓)。"
     "搜索引擎能渲染 JS、绕过部分反爬,常能把政务站救回来——停用不该是终点。默认开"),
    ...
    ("autopilot_health_max", "自动体检:每轮最多测几个源", "自动运维", "int", False,
     "默认 0 = 不按条数限量,一轮把该测的都测完。限量交给下面的「单个维护任务时长上限」——"
     "条数上限是拍脑袋的,源一变多就覆盖不到。只在想刻意压慢时才填"),
    ("autopilot_task_budget_seconds", "单个维护任务时长上限(秒)", "自动运维", "int", False,
     "真正该设的护栏:某一步跑到这么久就收尾,没处理完的下一轮接着做"
     "(定位/体检都按『最久没处理的优先』排,不会有源被永久跳过)。默认 3600;0=不限时"),
    ("autopilot_locate_days", "自动定位栏目周期(天)", "自动运维", "int", False, "默认 7"),
    ("autopilot_locate_max", "自动定位:每轮最多几个站", "自动运维", "int", False,
     "默认 0 = 不按条数限量,一轮把没定位的都定位完(受时长上限约束)"),
    ...
    ("engine_on_topic_min", "引擎切题率下限", "找源与覆盖", "float", False,
     ...
     "中文找源词返回过境外咨询公司、外国税务机关,域名却都合法——不看切题率拦不住,"
    ...
    ("query_evolution_enabled", "启用找源词进化", "找源与覆盖", "bool", False,
     ...
     "关掉则回到静态清单(config/discovery_sec.yaml 里写什么就跑什么)。默认开"),
    ("term_weak_lift", "限定词增益低于多少算拖后腿", "找源与覆盖", "float", False,
     "增益 = 组合每轮产出 / 锚点单独跑的每轮产出。「限定词+锚点」若不如「锚点」单独搜,增益就 <1。"
    ...
    ("prospect_engines", "主动找源:用哪些搜索引擎", "找源与覆盖", "str", False,
     ...
     "搜狗微信建议留着——大量一手通报发在公众号里;百度反爬较狠,可视情况去掉。"
     "这一项由「引擎自检并自动只留可用的」每 3 天自动维护,一般不用手改"),
    ("prospect_wechat_resolve_max", "主动找源:单轮解析几个公众号", "找源与覆盖", "int", False,
     "搜到的公众号文章要抓一次才知道属于哪个号,每条一次网络请求,按此值封顶。"
     "执法通报大多发在公众号里,一轮搜狗能带回上百个号,给太小会把最该收的一类源挡在门外。默认 120"),
    ...
    ("prospect_column_hint_max", "已有源站点:每站最多补几个栏目", "找源与覆盖", "int", False,
     "站点已有源≠该栏目已在采。找源命中的页面会反推出该站漏采的栏目并落库,每站上限。"
     "实测工信部一轮就命中 600 个页面,给太小等于白丢。默认 30;0=关闭"),
    ...
    ("probe_llm_enabled", "候选源 LLM 相关度初评", "找源与覆盖", "bool", False,
     "抓候选站首页抽样标题让模型判『是否持续产出本需求相关内容』(0-1,提示词来自画像)。"
    ...
    ("coverage_window_days", "覆盖度统计窗口(天)", "找源与覆盖", "int", False,
     "按画像覆盖维度统计近这么多天的记录数以判断覆盖空白。默认 90"),
    ("coverage_min_events", "覆盖度:低于几条算空白", "找源与覆盖", "int", False,
     "某维度取值在统计窗口内记录数低于此值即判为『覆盖空白』,自动生成该方向的找源词。默认 3"),
    ...
    ("semantic_recall_threshold", "语义去重阈值", "去重", "float", False, "记录摘要向量余弦相似度≥此值判疑似同一记录,0-1,默认 0.88"),
    ("fingerprint_window_days", "记录去重时间窗(天)", "去重", "int", False, "同主体同类型记录在此天数内视为同一记录(画像 record.dedup.window_days 可覆盖),默认 14"),
